...
#include <poppler-qt5.h> // For PDF outline parsing
#include <QDomDocument> // For PDF outline DOM parsing
#include <QRegExp> // For regex parsing in Qt5
...
void MainWindow::loadPdfOutline() {
    ...
    // Try to get the outline from the PDF document using Qt5 compatible API
    QDomDocument* outlineDoc = pdfDoc->toc();
    
    if (!outlineDoc || outlineDoc->isNull()) {
        ...
        if (outlineDoc) delete outlineDoc;
    } else {
        // Process the actual PDF outline from DOM
        // In Qt5 Poppler, the toc() returns a flat XML structure where each element is an outline item
        QDomElement root = outlineDoc->documentElement();
        if (!root.isNull()) {
            // The root element itself is the first outline item, process it and its siblings
            processQt5OutlineElement(root, nullptr);
        }
        delete outlineDoc;
    }
...
void MainWindow::processQt5OutlineElement(const QDomElement& element, QTreeWidgetItem* parentItem) {
    if (element.isNull()) return;
    
    // Process current element as an outline item
    ...
    // The element tag name is the title in Qt5 Poppler
    QString title = element.tagName();
    item->setText(0, title);
    
    // Parse the Destination attribute to get page number
    QString destination = element.attribute("Destination");
    if (!destination.isEmpty()) {
        // Qt5 Poppler destination format: "3;pageNumber;x;y;..." where the second number is the page
        QStringList parts = destination.split(';');
        if (parts.size() >= 2) {
            bool ok;
            int pageNumber = parts[1].toInt(&ok);
            if (ok && pageNumber > 0) {
                item->setData(0, Qt::UserRole, pageNumber); // Store 1-based page number
            }
        }
    }
    ...
    if (!nextSibling.isNull()) {
        processQt5OutlineElement(nextSibling, parentItem);
    }
}

void MainWindow::processOutlineElement(const QDomElement& element, QTreeWidgetItem* parentItem) {
    if (element.isNull()) return;
    ...
    for (int i = 0; i < children.count(); ++i) {
        ...
        if (childNode.isElement()) {
            ...
            // Set the title from the element
            ...
            if (title.isEmpty()) {
                title = QString("Item %1").arg(i + 1);
            }
            item->setText(0, title);
            
            // Try to get the page number from the destination
            QString destination = childElement.attribute("Destination");
            if (!destination.isEmpty()) {
                // Parse page number from destination string
                QRegExp pageRegex("page=(\\d+)");
                if (pageRegex.indexIn(destination) != -1) {
                    int pageNumber = pageRegex.cap(1).toInt();
                    if (pageNumber > 0) {
                        item->setData(0, Qt::UserRole, pageNumber);
                    }
                }
            }
            
            // Process child elements recursively
            processOutlineElement(childElement, item);
        }
    }
}
